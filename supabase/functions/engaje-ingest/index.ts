  ...
  try {
    ...
    if (dataset === 'ads') {
      const rows = sourceRows.map((row) => {
        ...
        return {organization_id: integration.organization_id, integration_id: integration.id, metric_date: date(row.metric_date), platform, account_id: text(row.account_id, 'account_id'), campaign_id: campaignId, campaign_name: text(row.campaign_name ?? campaignId, 'campaign_name'), adset_id: optionalText(row.adset_id, 240), ad_id: text(row.ad_id ?? `${campaignId}:aggregate`, 'ad_id'), currency: currency(row.currency), spend: numberValue(row.spend, 'spend', false), revenue: numberValue(row.revenue, 'revenue', true, false), impressions: integer(row.impressions, 'impressions'), clicks: integer(row.clicks, 'clicks'), page_views: integer(row.page_views, 'page_views'), leads: integer(row.leads, 'leads'), checkouts: integer(row.checkouts, 'checkouts'), purchases: integer(row.purchases, 'purchases'), attribution_window: text(row.attribution_window ?? 'source_default', 'attribution_window', 80), synced_at: now};
      });

...
export default async function Page() {
  ...
  const [{data: connections, error: connectionsError}, {data: organizations, error: organizationsError}, {data: alerts, error: alertsError}] = await Promise.all([
    ...
    db.from('integration_alerts').select('id,organization_id,code,severity,title,detail,last_seen_at').eq('status','open').order('last_seen_at',{ascending:false}).limit(50),
  ]);
  ...
  return <div className="space-y-6">
    ...
    <Card className={`space-y-4 ${attention?'border-amber-500/30':'border-emerald-500/20'}`}>
      <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="eyebrow mb-2">Monitoramento automático</p><h2 className="text-lg font-semibold">Saúde das integrações</h2><p className="muted mt-1 text-xs">Executado dentro do Supabase a cada 15 minutos.</p></div><span className={`connector-badge ${attention?'text-amber-300':'text-emerald-300'}`}>{attention?`${attention} alerta(s) aberto(s)`:'Tudo saudável'}</span></div>
      {attention?<div className="grid gap-2 lg:grid-cols-2">{alerts?.map((alert)=>{const organization=orgById.get(alert.organization_id);return <div className={`rounded-xl border p-4 ${alert.severity==='critical'?'border-red-500/25 bg-red-500/5':'border-amber-500/25 bg-amber-500/5'}`} key={alert.id}><div className="flex items-start gap-3"><TriangleAlert className={alert.severity==='critical'?'text-red-300':'text-amber-300'} size={18} /><div className="min-w-0"><strong className="text-sm">{alert.title}</strong><p className="muted mt-1 text-xs">{organization?.name??'Agência'} · {alert.code}</p>{alert.detail?<p className="mt-2 text-xs text-zinc-300">{alert.detail}</p>:null}<p className="muted mt-2 text-[11px]">Detectado em {new Date(alert.last_seen_at).toLocaleString('pt-BR')}</p></div></div></div>;})}</div>:<div className="rounded-xl border border-emerald-500/15 bg-emerald-500/5 p-4 text-sm text-emerald-200">Nenhuma conexão com erro, sincronização travada ou fonte desatualizada.</div>}
    </Card>

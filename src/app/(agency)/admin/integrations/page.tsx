...
function IntegrationCard({platform}: {platform: PlatformCard}) {
...
export default async function Page() {
  ...
  return <div className="space-y-8">
    ...
    <section className="space-y-4"><div><h2 className="text-xl font-semibold">Canais e plataformas</h2><p className="muted mt-1 text-sm">Integrações oficiais organizadas pelo ecossistema de origem.</p></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{platforms.map((platform)=><IntegrationCard platform={platform} key={platform.key} />)}</div></section>
    <section className="space-y-4"><div><h2 className="text-xl font-semibold">Serviços de dados</h2><p className="muted mt-1 text-sm">Extratores utilizados para transportar dados das plataformas até o dashboard.</p></div><div className="grid gap-4 md:grid-cols-2">{dataServices.map((platform)=><IntegrationCard platform={platform} key={platform.key} />)}</div></section>
  </div>;
}

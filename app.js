...
function showModulePage(title,body){hideDashboardShell();$('module-page')?.remove();const el=document.createElement('div');el.id='module-page';el.className='dash-content';el.innerHTML=`<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px"><button class="btn-sm-ghost" onclick="nav('dashboard')">← Dashboard</button><h2 style="margin:0;font-size:18px">${esc(title)}</h2></div>${body}`;$('main-area')?.appendChild(el);}
...
async function showAccounts(){showModulePage('Contas conectadas','<div class="section-card"><div class="empty-state"><div class="empty-title">Carregando...</div></div></div>');const found=await tryTable(['accounts'],'client_id=eq.'+encodeURIComponent(ST.clientId)+'&limit=100');showModulePage('Contas conectadas',`<div class="section-card"><div class="section-hdr"><span class="section-title">Contas do cliente atual</span>${isSuperAdmin()?'<button class="btn-sm-prim" id="sync-windsor-btn" onclick="syncWindsor()">Sincronizar Windsor agora</button>':''}</div>${found?.data?.length?objectTable(found.data):'<div class="empty-state"><div class="empty-title">Nenhuma conta retornada</div><div class="empty-desc">Este cliente ainda não possui conta social vinculada.</div></div>'}</div>`);}
async function syncWindsor(){
  if(!isSuperAdmin())return toast('Apenas o superadministrador pode sincronizar.','error');
  const btn=$('sync-windsor-btn');if(btn){btn.disabled=true;btn.textContent='Sincronizando...';}
  try{
    ...
    toast(`Sincronização concluída: ${d.posts} posts, ${d.images} imagens.`);await renderDashboard();
  }catch(e){toast(e.message,'error');}finally{if(btn){btn.disabled=false;btn.textContent='Sincronizar Windsor agora';}}
}

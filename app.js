...
function applyAvailability(){
  ...
  const enabled={overview:true,instagram:ST.available.organic.includes('instagram'),facebook:ST.available.organic.some(p=>p==='facebook_organic'||p==='facebook'),paid:ST.available.paid.length>0,audience:ST.available.audience,conversions:ST.available.conversions};
  const tabs=qsa('.dash-tab'),defs=[['overview','Visão Geral'],['instagram','Instagram orgânico'],['facebook','Facebook orgânico'],['paid','Meta Ads'],['audience','Audiência'],['conversions','Conversões']];
  tabs.forEach((tab,i)=>{const d=defs[i];if(!d)return;tab.dataset.tab=d[0];tab.textContent=d[1];tab.onclick=()=>setTab(d[0],tab);});
  tabs.forEach(tab=>tab.style.display=enabled[tab.dataset.tab]?'flex':'none');
  if(!enabled[ST.activeTab]){ST.activeTab='overview';tabs.forEach(t=>t.classList.toggle('active',t.dataset.tab==='overview'));qsa('[id^="tab-"]').forEach(t=>t.style.display=t.id==='tab-overview'?'block':'none');}
...
function tabElementId(tab){return tab==='instagram'?'tab-content':tab==='facebook'?'tab-competitors':'tab-'+tab;}
function setTab(tab,el){ST.activeTab=tab;qsa('.dash-tab').forEach(t=>t.classList.remove('active'));el?.classList.add('active');qsa('[id^="tab-"]').forEach(t=>t.style.display='none');const te=$(tabElementId(tab));if(te)te.style.display='block';renderTab(tab);}
function renderTab(tab){
  if(tab==='overview')return;
  if(tab==='instagram')return renderOrganicTab('instagram','tab-content');
  if(tab==='facebook')return renderOrganicTab('facebook_organic','tab-competitors');
  if(tab==='paid')return renderPaidTab(false);
  if(tab==='audience')return renderAudienceTab();
  if(tab==='conversions')return renderPaidTab(true);
}
async function renderOrganicTab(platform,id){
  const el=$(id);if(!el)return;el.innerHTML='<div class="empty-state"><div class="empty-title">Carregando...</div></div>';
  const posts=await loadPostsRange(currentRange(),platform);const s=summary(posts),title=platformName(platform);
  const cards=posts.map(p=>{const img=p.thumbnail_url||p.media_url,r=getReach(p.metrics),e=getEng(p.metrics),link=safeUrl(p.permalink);return `<article class="organic-card"${link!=='#'?` onclick="window.open('${esc(link)}','_blank')"`:''}>${img?`<img src="${esc(safeUrl(img))}" alt="Criativo de ${esc(title)}" loading="lazy" onerror="this.closest('.organic-card').classList.add('image-error');this.remove()">`:'<div class="organic-placeholder">Sem imagem disponível</div>'}<div class="organic-body"><div class="organic-meta">${esc(new Date(p.published_at).toLocaleDateString('pt-BR'))} · ${esc(p.media_type||'Post')}</div><div class="organic-caption">${esc((p.caption||'Sem legenda').slice(0,120))}</div><div class="organic-stats"><span><b>${fmtK(r)}</b> alcance</span><span><b>${fmtK(getImpressions(p.metrics))}</b> impressões</span><span><b>${fmtK(e)}</b> interações</span><span><b>${fmtPct(r?e/r*100:0)}</b> ER</span></div></div></article>`;}).join('');
  el.innerHTML=`<div class="platform-summary"><div><small>Canal</small><strong>${esc(title)}</strong></div><div><small>Publicações</small><strong>${fmt(s.posts)}</strong></div><div><small>Alcance</small><strong>${fmtK(s.reach)}</strong></div><div><small>Impressões</small><strong>${fmtK(s.impressions)}</strong></div><div><small>Interações</small><strong>${fmtK(s.engagement)}</strong></div><div><small>Taxa de engajamento</small><strong>${fmtPct(s.er)}</strong></div></div><div class="section-card"><div class="section-hdr"><span class="section-title">Conteúdos publicados (${posts.length})</span></div>${posts.length?`<div class="organic-grid">${cards}</div>`:'<div class="empty-state"><div class="empty-title">Sem conteúdo no período</div><div class="empty-desc">A plataforma possui histórico, mas não há publicações neste intervalo.</div></div>'}</div>`;
}

async function renderPaidTab(conversionsOnly=false){
  const id=conversionsOnly?'tab-conversions':'tab-paid',el=$(id);if(!el)return;el.innerHTML='<div class="empty-state"><div class="empty-title">Carregando Meta Ads...</div></div>';
  const range=currentRange(),q='meta_ad_metrics?client_id=eq.'+encodeURIComponent(ST.clientId)+'&date_start=gte.'+isoDate(range.start)+'&date_start=lt.'+isoDate(range.end)+'&select=*&order=date_start.desc&limit=1000';
  try{const rows=await rest(q)||[];if(!rows.length){el.innerHTML='<div class="section-card"><div class="empty-state"><div class="empty-title">Sem dados de Meta Ads</div><div class="empty-desc">Conecte e vincule uma conta de anúncios Meta a este cliente.</div></div></div>';return;}const total=k=>rows.reduce((n,r)=>n+num(r[k]),0),spend=total('spend'),results=total('results'),impressions=total('impressions'),clicks=total('clicks');const data=conversionsOnly?rows.filter(r=>num(r.results)>0):rows;el.innerHTML=`<div class="platform-summary"><div><small>Investimento</small><strong>${spend.toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}</strong></div><div><small>Resultados</small><strong>${fmt(results)}</strong></div><div><small>Impressões</small><strong>${fmtK(impressions)}</strong></div><div><small>Cliques</small><strong>${fmtK(clicks)}</strong></div><div><small>Custo por resultado</small><strong>${results?(spend/results).toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'—'}</strong></div></div><div class="section-card">${objectTable(data)}</div>`;}catch(e){el.innerHTML='<div class="empty-state"><div class="empty-title">Falha ao carregar Meta Ads</div><div class="empty-desc">'+esc(e.message)+'</div></div>';}
}

async function renderAudienceTab(){
  const el=$('tab-audience');if(!el)return;const range=currentRange();el.innerHTML='<div class="empty-state"><div class="empty-title">Carregando audiência...</div></div>';
  try{const rows=await rest('account_snapshots?client_id=eq.'+encodeURIComponent(ST.clientId)+'&snapshot_date=gte.'+isoDate(range.start)+'&snapshot_date=lt.'+isoDate(range.end)+'&select=*&order=snapshot_date.desc&limit=1000')||[];el.innerHTML=rows.length?`<div class="section-card"><div class="section-hdr"><span class="section-title">Evolução da audiência</span></div>${objectTable(rows)}</div>`:'<div class="section-card"><div class="empty-state"><div class="empty-title">Sem dados de audiência</div><div class="empty-desc">Ainda não existem snapshots de seguidores para este cliente.</div></div></div>';}catch(e){el.innerHTML='<div class="empty-state"><div class="empty-title">Falha ao carregar audiência</div><div class="empty-desc">'+esc(e.message)+'</div></div>';}
}
...
async function nav(page){
  ...
  if(page==='content'){restoreDashboardShell();const name=ST.available.organic.includes('instagram')?'instagram':'facebook';const tab=qs('.dash-tab[data-tab="'+name+'"]');setTab(name,tab);return;}

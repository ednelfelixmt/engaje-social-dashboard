...
function showModulePage(title,body){hideDashboardShell();$('module-page')?.remove();const el=document.createElement('div');el.id='module-page';el.className='dash-content';el.innerHTML=`<div class="module-page-header"><button class="btn-sm-ghost module-back" onclick="nav('dashboard')">← Dashboard</button><h2>${esc(title)}</h2></div>${body}`;$('main-area')?.appendChild(el);}
...
async function nav(page){
  ...
  if(page==='reports')return showReports();if(page==='clients')return showClients();if(page==='new-client')return showNewClient();if(page==='accounts')return showAccounts();if(page==='settings')return showSettings();
...
async function showAccounts(syncResult=null){showModulePage('Contas conectadas','<div class="section-card"><div class="empty-state"><div class="empty-title">Carregando...</div></div></div>');const found=await tryTable(['accounts'],'client_id=eq.'+encodeURIComponent(ST.clientId)+'&limit=100');const result=syncResult?`<div class="sync-result success" role="status"><strong>✓ Sincronização concluída com sucesso</strong><span>${fmt(syncResult.posts||0)} publicações processadas · ${fmt(syncResult.metrics||0)} métricas atualizadas · ${fmt(syncResult.images||0)} imagens armazenadas</span><small>Dados recebidos do Windsor e gravados no Supabase.</small></div>`:'';showModulePage('Contas conectadas',`${result}<div class="section-card"><div class="section-hdr accounts-header"><div><span class="section-title">Contas do cliente atual</span><small>${esc(ST.clientName||'Cliente não selecionado')}</small></div>${isSuperAdmin()?'<button class="btn-sm-prim sync-action" id="sync-windsor-btn" onclick="syncWindsor()">Sincronizar Windsor agora</button>':''}</div><div id="sync-progress" class="sync-progress" aria-live="polite"></div>${found?.data?.length?objectTable(found.data):'<div class="empty-state"><div class="empty-title">Nenhuma conta vinculada a este cliente</div><div class="empty-desc">A sincronização geral pode ter funcionado, mas o Windsor não retornou uma conta correspondente para este cliente.</div></div>'}</div>`);}
